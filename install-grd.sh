...
# install-grd enabled sshd as a unit, but without these configs the grd ssh door ran STOCK
# Fedora sshd (password-auth posture unhardened) and regenerated host keys every recreation
# (MITM-warning churn). Byte-equivalent to the xrdp install.sh drop-ins. ssh is :22, tailnet-only.
