...
`entrypoint.sh` is from fedora-dev (nested rootless podman + key-only sshd + tailscale + the
daily-claudebox machinery), MINUS the fail2ban/rsyslog ssh-brute-force apparatus — fedora-dev
publishes a public ssh door, this box does not (ssh is tailnet-only), so that apparatus guarded
a door never opened here; the public web door's brute-force defense is guacamole-auth-ban.
PART B layers the XFCE/xrdp desktop
...
| nftables | Fedora current | a | firewall backend (tailscaled via Netlink API, netavark default on F41+, the entrypoint `fd_tailnet_guard` that fences ssh/RDP/VNC to the tailnet). No iptables — verified unnecessary |
...
| entrypoint.sh | PID 1 (root): seeds `core`'s password from RDP_PW; syncs ssh keys; mints the Guacamole TLS keystore; brings up the loopback MariaDB engine + provisions DB-auth/TOTP via the shared `guac-db-provision.sh`; supervises sshd (tailnet-only) + tailscaled + the rootless podman socket + inotify rebuild-watcher + daily-tick + first-boot live-clone-or-seed + eager claudebox assemble + xrdp-sesman/xrdp + **mariadbd** + guacd + Tomcat + the optional RFB_PW VNC mirror + the cloud-sync/vault-gitsync helpers; single `pgrep`/`kill -0` watchdog; SIGTERM trap for clean shutdown |
